...
trend-viewer 자체는 Python 표준 라이브러리만 사용하며 API 키 없이 휴리스틱 분석까지 동작한다. 로컬 ocx 프록시(gpt-5.6-luna)가 떠 있으면 자동으로 LLM 클러스터링·브리핑으로 강화되고, 없으면 조용히 휴리스틱으로 폴백한다.
...
| `TREND_ANALYSIS_MODEL` | `gpt-5.6-luna` | 분석 클러스터링·브리핑 모델 |
